...
class STRLProcessorNoNum(BaseProcessor):
    ...
    def __init__(self, logger: BaseLogger, **kwargs) -> None:
        ...
        self._stop_pattern = re.compile(f'{self.TERMINATING_STRING}.')
    ...
    def _steps_to_text(self,
        ...
        text = ", ".join([f'{self._step_to_text(step)}'
                          for _ , step in enumerate(steps, start=1)])
        if add_terminating_string:
            text += f", {self.TERMINATING_STRING}."
        return text
